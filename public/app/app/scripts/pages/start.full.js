...
/**
 *? Mounts the Clerk frontend API script and initializes ClerkJS.
 ** @returns {Promise<void>} Resolves when Clerk is initialized.
 * */
...
async function lscCmdbar() {
    await lsc("app/scripts/functions/cmdbar.js", "cmdbar.js")
...
async function updateUser(user) {
    ...
    if (funds_updated_at) {
        $(`[start_crd="preview"]`).hide();
        $(`[start_crd="extend_proxies"]`).show();
    }
    if (member) {
        $(`[start_crd="member_discount"]`).hide();
    }
    return;
}
...
async function clerkResolved() {
    try {
        ...
        // Set click events
        $(`.cmdbhelp--nav, .cmdbhelp--crd`).off('click').click(async function () {
