...
async function clerkResolved() {
    ...
    $(`.cmdbhelp--cta`).off('click').click(async function () {
        window.CommandBar.toggleHelpHub();
    })
    // Wait for 10 seconds before starting the loop
    let user = null;
    let attempts = 0;
    const maxAttempts = 12; // 6 attempts at 10-second intervals = 50 seconds

    while (!user && attempts < maxAttempts) {
        if (attempts > 1 && attempts <= 6) {
            $(`[cta_wait="${attempts - 1}"]`).hide();
            $(`[cta_wait="${attempts}"]`).show();
        }
    ...
    await redirect(`_self`, `/app`, false)
