 ...
 */
async function getApiKeys(token, format = 'visible', master = true) {
    if (!token) {
        token = await supaToken();
    }
    let apiKeysEndpoint = new URL(`https://cmd.illusory.io/v1/apikeys?format=${format}&master=${master}`);
